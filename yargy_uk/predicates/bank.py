...
from yargy_uk.tokenizer import INT
from yargy_uk.token import (
    is_tag_token,
    is_morph_token
)
...
def mock_context():
    from yargy_uk.parser import Context
...
class vesum_tag(ParameterPredicateScheme):
    def activate(self, context):
        return VesumTagPredicate(self.value)
...
class VesumTagPredicate(ParameterPredicate):
    @morph_required
    def __call__(self, token):
        return any(
            self.value in _['tags']
            for _ in token.vesum_records
        )
