...
from yargy_uk.visitor import Visitor
from yargy_uk.dot import (
    style,
    DotTransformator,
    BLUE,
    ORANGE,
...
from yargy_uk.predicates import is_predicate
...
def max_bound(item, count, reverse=False):
    from yargy_uk.api import rule, or_
...
def repeatable(item, reverse=False):
    from yargy_uk.api import forward, or_, rule
...
def optional(item, reverse=False):
    from yargy_uk.api import or_, empty
...
def repeatable_optional(item, reverse_repeatable=False, reverse_optional=False):
    from yargy_uk.api import forward, or_, rule, empty
